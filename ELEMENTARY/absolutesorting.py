...
def checkio(numbers_array):
    # create list where the first item of each member is the absolute value
    i = map(lambda x: [abs(x), x], numbers_array)
    # sort the list accoring the the absolute value
    i.sort()
    # return list of non absolute values as secong item of the list i
    return map(lambda x: x[1], i)
